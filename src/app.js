...
function renderPicks(){
  const el=document.getElementById("picksTable"),round=getRound(sharedRoundId);if(!round){el.innerHTML='<div class="empty">请先新建比赛日</div>';return;}
  const picks=state.picks.filter(p=>p.roundId===round.id);if(!picks.length){el.innerHTML='<div class="empty">当前比赛日暂无选股</div>';return;}
  el.innerHTML=`<div class="notice" style="margin-bottom:12px">T日 ${fmtDate(round.pickDate)}；T+1 ${fmtDate(round.buyDate)}；T+2 ${fmtDate(round.sellDate)}</div><div class="table-wrap"><table><thead><tr><th>选手</th><th>股票</th><th>板块</th><th>当前价</th><th>技术评分</th><th>理由</th><th>操作</th></tr></thead><tbody>${picks.map(p=>{const person=state.participants.find(x=>x.id===p.participantId);return`<tr><td>${escapeHtml(person?.name||"未知")}</td><td><strong>${escapeHtml(p.stockCode)}</strong><br>${escapeHtml(p.stockName||"")}</td><td>${escapeHtml(p.sector||"-")}</td><td>${fmtPrice(p.currentPrice)}${p.marketError?`<div>${errorButton(p.marketError)}</div>`:""}</td><td>${p.analysisSummary?`<span class="badge ${p.analysisSummary.score>=76?"success":p.analysisSummary.score<58?"warning":""}">${p.analysisSummary.level} · ${p.analysisSummary.score}/120</span>`:'<span class="badge">未分析</span>'}</td><td>${escapeHtml(p.reason||"-")}</td><td><div class="actions"><button class="btn btn-small" onclick="openTechnical('${p.id}')">技术分析</button><button class="btn btn-small" onclick="refreshPickQuote('${p.id}',true)">行情</button><button class="btn btn-small" onclick="openPickModal('${p.id}')">编辑</button><button class="btn btn-small btn-danger" onclick="deletePick('${p.id}')">删除</button></div></td></tr>`;}).join("")}</tbody></table></div>`;
}
...
async function analyzePick(id,openAfter=true){const pick=state.picks.find(p=>p.id===id),roundInfo=getRound(pick?.roundId);if(!pick||!roundInfo)return;try{if(openAfter){openModal("technicalModal");document.getElementById("technicalBody").innerHTML='<div class="empty"><i class="spin">⟳</i> 正在获取历史日线并计算…</div>';}const start=new Date(roundInfo.pickDate);start.setDate(start.getDate()-900);const startDate=start.toISOString().slice(0,10);const [stock,bench]=await Promise.all([apiJSON(`/api/history?code=${pick.stockCode}&start=${startDate}&end=${roundInfo.pickDate}&limit=420`),apiJSON(`/api/history?code=000001&market=sh&start=${startDate}&end=${roundInfo.pickDate}&limit=420`)]);const full=buildAnalysis(stock.data.rows,bench.data.rows);full.provider=stock.data.provider;full.benchmarkProvider=bench.data.provider;technicalCache.set(id,full);pick.analysisSummary={score:full.score,level:full.level,dataDate:full.dataDate,rsi14:full.rsi14,macd:full.macd,return5:full.return5,return20:full.return20,relative20:full.relative20,generatedAt:nowISO()};pick.updatedAt=nowISO();markChanged("技术分析已更新");if(openAfter)renderTechnicalModal(pick,full);}catch(error){pick.marketError=`技术分析：${error.message}`;pick.updatedAt=nowISO();markChanged("技术分析失败");if(openAfter)document.getElementById("technicalBody").innerHTML=`<div class="notice online-warning">分析失败 ${errorButton(error.message)}</div>`;}}
