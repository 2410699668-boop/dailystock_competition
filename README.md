...
本版本在 Supabase 在线共创结构中恢复了旧版的选手分析模块：

- 选手总结页重新增加“选手风格画像”；
- 根据正式成绩、胜率、波动、最大回撤、板块集中度和技术评分生成风格说明；
- 重新增加个人累计收益折线图，按正式比赛日复利计算；
- 增加最大回撤、波动率、最长连胜等摘要指标；
- 每日选股页面恢复“分析全部”按钮；
- 批量分析最多同时处理 2 只股票，完成后统一同步到 Supabase，减少多人在线版的重复写入；
- 单只分析失败时保留其他股票的分析结果，并通过“错误！”按钮查看详情。
