...
var GoTrueAdminApi = class {
   ...
   * This function should only be called on a server. Never expose your `service_role` key in the browser.
   */
  async _getOAuthClient(clientId2) {
    try {
      return await _request(this.fetch, "GET", `${this.url}/admin/oauth/clients/${clientId2}`, {
        headers: this.headers,
        xform: (client) => {
          return { data: client, error: null };
        }
   ...
   * This function should only be called on a server. Never expose your `service_role` key in the browser.
   */
  async _updateOAuthClient(clientId2, params) {
    try {
      return await _request(this.fetch, "PUT", `${this.url}/admin/oauth/clients/${clientId2}`, {
        body: params,
        headers: this.headers,
        xform: (client) => {
          return { data: client, error: null };
   ...
   * This function should only be called on a server. Never expose your `service_role` key in the browser.
   */
  async _deleteOAuthClient(clientId2) {
    try {
      await _request(this.fetch, "DELETE", `${this.url}/admin/oauth/clients/${clientId2}`, {
        headers: this.headers,
        noResolveJson: true
      });
      return { data: null, error: null };
   ...
   * This function should only be called on a server. Never expose your `service_role` key in the browser.
   */
  async _regenerateOAuthClientSecret(clientId2) {
    try {
      return await _request(this.fetch, "POST", `${this.url}/admin/oauth/clients/${clientId2}/regenerate_secret`, {
        headers: this.headers,
        xform: (client) => {
          return { data: client, error: null };
        }
...
// src/app.js
var LOCAL_CACHE_KEY = "stockCompetitionCollabCache_v1";
var CLIENT_ID_KEY = "stockCompetitionCollabClientId_v1";
var CLIENT_NAME_KEY = "stockCompetitionCollabClientName_v1";
var SHARED_ROUND_KEY = "stockCompetitionCollabRound_v1";
var TECH_LAYER_KEY = "stockCompetitionTechLayers_v2";
var nowISO = () => (/* @__PURE__ */ new Date()).toISOString();
var uid = (prefix) => `${prefix}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 9)}`;
var escapeHtml = (value) => String(value ?? "").replace(/[&<>'"]/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" })[char]);
var numeric = (value) => value === "" || value === null || value === void 0 ? null : Number.isFinite(Number(value)) ? Number(value) : null;
var fmtPrice = (value) => numeric(value) === null ? "-" : Number(value).toLocaleString("zh-CN", { minimumFractionDigits: 2, maximumFractionDigits: 3 });
var fmtPct = (value) => numeric(value) === null ? "-" : `${Number(value) >= 0 ? "+" : ""}${Number(value).toFixed(2)}%`;
var fmtDate = (value) => value ? String(value).slice(0, 10) : "-";
var fmtDateTime = (value) => {
  if (!value) return "-";
  const d = new Date(value);
  return Number.isNaN(d.getTime()) ? String(value) : d.toLocaleString("zh-CN", { hour12: false });
};
var pctClass = (value) => numeric(value) === null ? "" : Number(value) > 0 ? "positive" : Number(value) < 0 ? "negative" : "";
var activeParticipants = () => state.participants.filter((item) => item.status !== "inactive");
var getRound = (id) => state.rounds.find((item) => item.id === id);
var getPick = (roundId, participantId) => state.picks.find((item) => item.roundId === roundId && item.participantId === participantId);
function defaultState() {
  return {
    version: 1,
    settings: { competitionName: "\u6BCF\u65E5\u9009\u80A1\u64C2\u53F0", allowDuplicate: true, unsubmittedReturn: 0, pointRule: "rank", ruleNote: "T\u65E5\u9009\u80A1\uFF0CT+1\u65E5\u5F00\u76D8\u4EF7\u4E70\u5165\uFF0CT+2\u65E5\u5F00\u76D8\u4EF7\u5356\u51FA\uFF1B\u6536\u76CA\u7387=(T+2\u5F00\u76D8\u4EF7\xF7T+1\u5F00\u76D8\u4EF7-1)\xD7100%\u3002" },
    participants: [],
    rounds: [],
    picks: [],
    meta: { settingsUpdatedAt: (/* @__PURE__ */ new Date(0)).toISOString(), deleted: { participants: {}, rounds: {}, picks: {} } }
  };
}
function normalizeState(input) {
  ...
  const output = {
    ...
    picks: Array.isArray(source.picks) ? source.picks.map((item) => ({ ...item, updatedAt: item.updatedAt || item.createdAt || nowISO() })) : [],
...
function loadLocal() {
  try {
    return normalizeState(JSON.parse(storageGet(LOCAL_CACHE_KEY) || "null"));
  } catch {
    return defaultState();
  }
}
function saveLocal() {
  storageSet(LOCAL_CACHE_KEY, JSON.stringify(state));
}
function clientId() {
  let id = storageGet(CLIENT_ID_KEY);
  if (!id) {
    id = `visitor_${crypto.randomUUID?.() || uid("v")}`;
    storageSet(CLIENT_ID_KEY, id);
  }
  return id;
}
var visitorName = storageGet(CLIENT_NAME_KEY) || `\u8BBF\u5BA2-${clientId().slice(-4).toUpperCase()}`;
var state = loadLocal();
...
var sharedRoundId = storageGet(SHARED_ROUND_KEY) || "";
var technicalCache = /* @__PURE__ */ new Map();
var techLayers = { ma5: true, ma10: false, ma20: true, ma30: false, ma50: false, ma90: false, ma105: false, ma180: false, macd: false, shIndex: false };
try {
  techLayers = { ...techLayers, ...JSON.parse(storageGet(TECH_LAYER_KEY) || "{}") };
} catch {
}
var APP_CONFIG = window.APP_CONFIG || {};
var SUPABASE_URL = String(APP_CONFIG.supabaseUrl || "").trim();
var SUPABASE_PUBLISHABLE_KEY = String(APP_CONFIG.supabasePublishableKey || "").trim();
var supabaseConfigured = /^https:\/\/.+\.supabase\.co$/i.test(SUPABASE_URL) && SUPABASE_PUBLISHABLE_KEY.length > 20;
var supabase = supabaseConfigured ? createClient(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, {
...
function mergeDeleted(a, b) {
  const out = { participants: {}, rounds: {}, picks: {} };
  for (const type of Object.keys(out)) {
    for (const src of [a?.[type] || {}, b?.[type] || {}]) for (const [id, time] of Object.entries(src)) {
      if (Date.parse(time || 0) > Date.parse(out[type][id] || 0)) out[type][id] = time;
    }
  }
  return out;
}
function mergeEntityArray(local, remote, type, deleted) {
  ...
  for (const item of [...remote || [], ...local || []]) {
    ...
    if (!current || entityTime(item) >= entityTime(current)) map.set(item.id, item);
  }
  return [...map.values()].filter((item) => Date.parse(deleted?.[type]?.[item.id] || 0) < entityTime(item));
}
function mergeStates(local, remote) {
  const l = normalizeState(local), r = normalizeState(remote);
  const deleted = mergeDeleted(l.meta.deleted, r.meta.deleted);
...
function setSyncStatus(kind, text) {
  const pill = document.getElementById("syncPill"), label = document.getElementById("syncText");
  if (!pill || !label) return;
  pill.className = `sync-pill ${kind}`;
  label.textContent = text;
}
function updateLastEditor(record) {
  const el = document.getElementById("lastEditorText");
  if (!el) return;
  el.textContent = record?.updatedAt ? `\u6700\u8FD1\u4FEE\u6539\uFF1A${record.updatedBy?.name || "\u533F\u540D\u8BBF\u5BA2"} \xB7 ${fmtDateTime(record.updatedAt)}` : "\u5C1A\u65E0\u5728\u7EBF\u4FEE\u6539\u8BB0\u5F55";
}
function isEditing() {
  return !!document.querySelector(".modal-backdrop.show") || ["INPUT", "TEXTAREA", "SELECT"].includes(document.activeElement?.tagName);
}
...
function configError() {
  return "\u5C1A\u672A\u914D\u7F6E Supabase\u3002\u8BF7\u5728 Netlify \u73AF\u5883\u53D8\u91CF\u4E2D\u586B\u5199 SUPABASE_URL \u548C SUPABASE_PUBLISHABLE_KEY\uFF0C\u5E76\u91CD\u65B0\u90E8\u7F72\u3002";
}
...
async function loadRemote({ initial = false, forceApply = false } = {}) {
  if (!supabase) {
    setSyncStatus("error", configError());
    return;
  }
  try {
    ...
    if (isEditing() && !forceApply) {
      pendingRemote = record;
      setSyncStatus("saving", "\u6709\u5176\u4ED6\u7528\u6237\u66F4\u65B0\uFF0C\u5B8C\u6210\u5F53\u524D\u7F16\u8F91\u540E\u5E94\u7528");
      return;
    }
    state = remote;
    saveLocal();
    renderAll();
    setSyncStatus("online", "\u5728\u7EBF \xB7 Supabase \u5DF2\u540C\u6B65");
  } catch (error) {
    setSyncStatus("offline", `\u79BB\u7EBF\u7F13\u5B58 \xB7 ${remoteErrorText(error)}`);
  }
}
function applyPendingRemote() {
  if (!pendingRemote || isEditing()) return;
  state = mergeStates(state, pendingRemote.state);
  remoteRevision = pendingRemote.revision || remoteRevision;
  updateLastEditor(pendingRemote);
  pendingRemote = null;
  saveLocal();
  renderAll();
  setSyncStatus("online", "\u5728\u7EBF \xB7 \u5DF2\u5E94\u7528\u5176\u4ED6\u7528\u6237\u4FEE\u6539");
}
function markChanged(message = "\u5DF2\u4FEE\u6539") {
  dirty = true;
  saveLocal();
  renderAll();
  setSyncStatus("saving", `${message} \xB7 \u7B49\u5F85\u540C\u6B65`);
...
async function saveRemote(retry = true) {
  ...
  syncing = true;
  setSyncStatus("saving", "\u6B63\u5728\u4FDD\u5B58\u5230 Supabase\u2026");
  ...
  try {
    ...
    if (remoteRevision <= 0) {
      ...
      if (error) {
        if (error.code === "23505" && retry) {
          const latest = await fetchRemoteRow();
          if (latest) {
            remoteRevision = latest.revision;
            state = mergeStates(state, latest.state);
            saveLocal();
            renderAll();
            syncing = false;
            dirty = true;
            return saveRemote(false);
          }
        }
        throw error;
      }
    ...
    const record = normalizeRemoteRow(row);
    remoteRevision = record.revision;
    dirty = false;
    updateLastEditor(record);
    setSyncStatus("online", "\u5728\u7EBF \xB7 Supabase \u5DF2\u540C\u6B65");
  } catch (error) {
    setSyncStatus("error", `\u540C\u6B65\u5931\u8D25 \xB7 ${remoteErrorText(error)}`);
    showToast("\u540C\u6B65\u5931\u8D25\uFF0C\u4FEE\u6539\u5DF2\u4FDD\u5B58\u5728\u672C\u673A\u7F13\u5B58");
  } finally {
    syncing = false;
  }
}
async function handleIncomingRow(row) {
  const record = normalizeRemoteRow(row);
  if (!record || record.revision <= remoteRevision) return;
  if (dirty) {
    remoteRevision = record.revision;
    state = mergeStates(state, record.state);
    saveLocal();
    renderAll();
    await saveRemote();
    return;
  ...
  if (isEditing()) {
    pendingRemote = record;
    setSyncStatus("saving", "\u6709\u5176\u4ED6\u7528\u6237\u66F4\u65B0\u5F85\u5E94\u7528");
    return;
  }
  state = record.state;
  remoteRevision = record.revision;
  ...
  renderAll();
  updateLastEditor(record);
  setSyncStatus("online", "\u5728\u7EBF \xB7 \u5DF2\u63A5\u6536\u5B9E\u65F6\u66F4\u65B0");
}
function startRealtime() {
  if (!supabase) return;
  realtimeChannel = supabase.channel("stock-competition-main").on("postgres_changes", { event: "INSERT", schema: "public", table: "competition_state", filter: "id=eq.main" }, (payload) => handleIncomingRow(payload.new)).on("postgres_changes", { event: "UPDATE", schema: "public", table: "competition_state", filter: "id=eq.main" }, (payload) => handleIncomingRow(payload.new)).subscribe((status) => {
    if (status === "SUBSCRIBED" && !dirty) setSyncStatus("online", "\u5728\u7EBF \xB7 Supabase \u5B9E\u65F6\u540C\u6B65");
    if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") setSyncStatus("offline", "\u5B9E\u65F6\u8FDE\u63A5\u6682\u4E0D\u53EF\u7528 \xB7 \u5C06\u5B9A\u65F6\u540C\u6B65");
  });
}
async function pollRemote() {
  ...
  try {
    const record = await fetchRemoteRow();
    if (record && record.revision > remoteRevision) await handleIncomingRow({ id: "main", revision: record.revision, payload: record.state, updated_at: record.updatedAt, updated_by: record.updatedBy });
  } catch {
    setSyncStatus("offline", "\u7F51\u7EDC\u6682\u65F6\u4E0D\u53EF\u7528 \xB7 \u4F7F\u7528\u672C\u673A\u7F13\u5B58");
  }
}
function ensureMeta() {
  state.meta = state.meta || {};
  state.meta.deleted = state.meta.deleted || { participants: {}, rounds: {}, picks: {} };
  for (const key of ["participants", "rounds", "picks"]) state.meta.deleted[key] = state.meta.deleted[key] || {};
}
function tombstone(type, id) {
  ensureMeta();
  state.meta.deleted[type][id] = nowISO();
}
...
function sortedRounds() {
  return [...state.rounds].sort((a, b) => String(b.pickDate).localeCompare(String(a.pickDate)));
}
function ensureSharedRound() {
  const valid = state.rounds.some((r) => r.id === sharedRoundId);
  if (!valid) sharedRoundId = sortedRounds()[0]?.id || "";
  storageSet(SHARED_ROUND_KEY, sharedRoundId);
}
function setSharedRound(id) {
  sharedRoundId = id || "";
  storageSet(SHARED_ROUND_KEY, sharedRoundId);
  renderRoundDependent();
}
...
function dailyRows(roundId) {
  ...
  const rows = participants.map((participant) => {
    const pick = getPick(roundId, participant.id);
    const info = returnInfo(pick);
    return { participant, pick, returnPct: pick ? info.value : Number(state.settings.unsubmittedReturn || 0), isTemporary: !!pick && info.isTemporary, isMissing: !!pick && info.value === null, isUnsubmitted: !pick };
  });
  rows.sort((a, b) => {
    const av = a.returnPct === null ? -Infinity : a.returnPct, bv = b.returnPct === null ? -Infinity : b.returnPct;
    return bv - av || a.participant.name.localeCompare(b.participant.name, "zh-CN");
  });
  rows.forEach((row, index) => {
    row.rank = index + 1;
    row.points = rows.length - index;
  });
  return rows;
}
function totalRows() {
  ...
  const rows = activeParticipants().map((participant) => {
    let equity = 1, points = 0, days = 0, wins = 0;
    for (const round2 of settled) {
      const row = dailyRows(round2.id).find((x) => x.participant.id === participant.id);
      if (!row || row.isTemporary || row.isMissing) continue;
      equity *= 1 + Number(row.returnPct || 0) / 100;
      points += row.points || 0;
      days += 1;
      if (row.rank === 1) wins += 1;
    }
    return { participant, compoundReturn: (equity - 1) * 100, points, days, wins };
  });
  rows.sort((a, b) => b.compoundReturn - a.compoundReturn || b.points - a.points);
...
function renderDashboard() {
  ...
  const settled = state.rounds.filter((r) => r.status === "settled").length;
  const stats = [
...
function overviewSVG() {
  ...
  const series = participants.map((p) => {
    let equity = 1;
    return { participant: p, points: rounds.map((round2) => {
      const row = dailyRows(round2.id).find((x2) => x2.participant.id === p.id);
      if (row && !row.isTemporary && !row.isMissing) equity *= 1 + Number(row.returnPct || 0) / 100;
      return (equity - 1) * 100;
    }) };
  });
  const vals = [0, ...series.flatMap((s) => s.points)];
  ...
  const grids = Array.from({ length: 5 }, (_, i) => {
    ...
    return `<line x1="${m.l}" y1="${yy}" x2="${width - m.r}" y2="${yy}" stroke="#e4e8f0"/><text x="${m.l - 7}" y="${yy + 3}" text-anchor="end" font-size="10" fill="#98a2b3">${v.toFixed(1)}%</text>`;
  }).join("");
  const paths = series.map((s) => {
    const color = s.participant.color || "#2563eb";
    const d = s.points.map((v, i) => `${i ? "L" : "M"}${x(i)},${y(v)}`).join(" ");
    return `<path d="${d}" fill="none" stroke="${color}" stroke-width="2.6" stroke-linecap="round"/>${s.points.map((v, i) => `<circle cx="${x(i)}" cy="${y(v)}" r="3.3" fill="#fff" stroke="${color}" stroke-width="2"><title>${escapeHtml(s.participant.name)} ${fmtDate(rounds[i].pickDate)} ${fmtPct(v)}</title></circle>`).join("")}`;
  }).join("");
...
function renderParticipants() {
  const el = document.getElementById("participantsTable");
  if (!state.participants.length) {
    el.innerHTML = '<div class="empty">\u6682\u65E0\u53C2\u8D5B\u8005</div>';
    return;
  }
...
function renderRounds() {
  ...
  if (!rounds.length) {
    el.innerHTML = '<div class="empty">\u6682\u65E0\u6BD4\u8D5B\u65E5</div>';
    return;
  }
...
function renderRoundSelects() {
  const rounds = sortedRounds();
  const html = rounds.length ? rounds.map((r) => `<option value="${r.id}">T ${fmtDate(r.pickDate)}\uFF5CT+1 ${fmtDate(r.buyDate)}\uFF5CT+2 ${fmtDate(r.sellDate)}${r.status === "settled" ? "\uFF08\u5DF2\u7ED3\u7B97\uFF09" : ""}</option>`).join("") : '<option value="">\u6682\u65E0\u6BD4\u8D5B\u65E5</option>';
  for (const id of ["roundSelectPicks", "roundSelectSettlement", "roundSelectDaily"]) {
    const el = document.getElementById(id);
    if (el) {
      el.innerHTML = html;
      el.value = sharedRoundId;
    }
  }
}
function renderPicks() {
  const el = document.getElementById("picksTable"), round2 = getRound(sharedRoundId);
  if (!round2) {
    el.innerHTML = '<div class="empty">\u8BF7\u5148\u65B0\u5EFA\u6BD4\u8D5B\u65E5</div>';
    return;
  }
  const picks = state.picks.filter((p) => p.roundId === round2.id);
  if (!picks.length) {
    el.innerHTML = '<div class="empty">\u5F53\u524D\u6BD4\u8D5B\u65E5\u6682\u65E0\u9009\u80A1</div>';
    return;
  }
  el.innerHTML = `<div class="notice" style="margin-bottom:12px">T\u65E5 ${fmtDate(round2.pickDate)}\uFF1BT+1 ${fmtDate(round2.buyDate)}\uFF1BT+2 ${fmtDate(round2.sellDate)}</div><div class="table-wrap"><table><thead><tr><th>\u9009\u624B</th><th>\u80A1\u7968</th><th>\u677F\u5757</th><th>\u5F53\u524D\u4EF7</th><th>\u6280\u672F\u8BC4\u5206</th><th>\u7406\u7531</th><th>\u64CD\u4F5C</th></tr></thead><tbody>${picks.map((p) => {
    const person = state.participants.find((x) => x.id === p.participantId);
    return `<tr><td>${escapeHtml(person?.name || "\u672A\u77E5")}</td><td><strong>${escapeHtml(p.stockCode)}</strong><br>${escapeHtml(p.stockName || "")}</td><td>${escapeHtml(p.sector || "-")}</td><td>${fmtPrice(p.currentPrice)}${p.marketError ? `<div>${errorButton(p.marketError)}</div>` : ""}</td><td>${p.analysisSummary ? `<span class="badge ${p.analysisSummary.score >= 76 ? "success" : p.analysisSummary.score < 58 ? "warning" : ""}">${p.analysisSummary.level} \xB7 ${p.analysisSummary.score}/120</span>` : '<span class="badge">\u672A\u5206\u6790</span>'}</td><td>${escapeHtml(p.reason || "-")}</td><td><div class="actions"><button class="btn btn-small" onclick="openTechnical('${p.id}')">\u6280\u672F\u5206\u6790</button><button class="btn btn-small" onclick="refreshPickQuote('${p.id}',true)">\u884C\u60C5</button><button class="btn btn-small" onclick="openPickModal('${p.id}')">\u7F16\u8F91</button><button class="btn btn-small btn-danger" onclick="deletePick('${p.id}')">\u5220\u9664</button></div></td></tr>`;
  }).join("")}</tbody></table></div>`;
}
function renderSettlement() {
  const el = document.getElementById("settlementTable"), round2 = getRound(sharedRoundId);
  if (!round2) {
    el.innerHTML = '<div class="empty">\u8BF7\u5148\u9009\u62E9\u6BD4\u8D5B\u65E5</div>';
    return;
  }
  const participants = activeParticipants();
  ...
  el.innerHTML = `<div class="table-wrap"><table><thead><tr><th>\u9009\u624B</th><th>\u80A1\u7968</th><th>T+1\u5F00\u76D8\u4EF7</th><th>T+2\u5F00\u76D8\u4EF7</th><th>\u5F53\u524D\u4EF7</th><th>\u6536\u76CA\u7387</th><th>\u72B6\u6001</th></tr></thead><tbody>${participants.map((person) => {
    const p = getPick(round2.id, person.id);
    if (!p) return `<tr><td>${escapeHtml(person.name)}</td><td>-</td><td>-</td><td>-</td><td>-</td><td>${fmtPct(state.settings.unsubmittedReturn)}</td><td><span class="badge danger">\u672A\u63D0\u4EA4</span></td></tr>`;
    const info = returnInfo(p);
    return `<tr><td>${escapeHtml(person.name)}</td><td>${escapeHtml(p.stockCode)} ${escapeHtml(p.stockName || "")}${p.marketError ? `<div>${errorButton(p.marketError)}</div>` : ""}</td><td><input style="width:100px" type="number" step="0.001" value="${p.buyPrice ?? ""}" onchange="updatePickPrice('${p.id}','buyPrice',this.value)"></td><td><input style="width:100px" type="number" step="0.001" value="${p.sellPrice ?? ""}" onchange="updatePickPrice('${p.id}','sellPrice',this.value)"></td><td><input style="width:100px" type="number" step="0.001" value="${p.currentPrice ?? ""}" onchange="updatePickPrice('${p.id}','currentPrice',this.value)"></td><td class="${pctClass(info.value)}">${fmtPct(info.value)}</td><td>${info.value === null ? '<span class="badge warning">\u5F85\u5F55\u5165</span>' : info.isTemporary ? '<span class="badge warning">\u4E34\u65F6\u6536\u76CA</span>' : '<span class="badge success">\u6B63\u5F0F\u4EF7\u683C</span>'}</td></tr>`;
  }).join("")}</tbody></table></div>`;
}
function renderDaily() {
  const round2 = getRound(sharedRoundId), el = document.getElementById("dailyTable"), chart = document.getElementById("dailyBarChart");
  if (!round2) {
    el.innerHTML = '<div class="empty">\u8BF7\u9009\u62E9\u6BD4\u8D5B\u65E5</div>';
    chart.innerHTML = "";
    return;
  }
  const rows = dailyRows(round2.id);
  chart.innerHTML = dailyBarSVG(rows, round2);
...
function dailyBarSVG(rows, round2) {
  const values = rows.filter((r) => numeric(r.returnPct) !== null);
  if (!values.length) return '<div class="empty">\u6682\u65E0\u53EF\u7ED8\u5236\u6210\u7EE9</div>';
  const width = 760, rowH = 42, height = Math.max(230, 60 + values.length * rowH), m = { l: 140, r: 70, t: 16, b: 34 }, iw = width - m.l - m.r;
  let min = Math.min(0, ...values.map((r) => r.returnPct)), max = Math.max(0, ...values.map((r) => r.returnPct));
  if (min === max) {
    min -= 1;
    max += 1;
  }
  ...
  min -= pad;
  max += pad;
  const x = (v) => m.l + (v - min) / (max - min) * iw, zero = x(0);
  const bars = values.map((r, i) => {
    const y = m.t + i * rowH + 6, xx = x(r.returnPct), left = Math.min(zero, xx), w = Math.max(2, Math.abs(xx - zero)), color = r.participant.color || "#2563eb";
    return `<text x="${m.l - 8}" y="${y + 16}" text-anchor="end" font-size="11" fill="#344054">\u7B2C${r.rank}\u540D ${escapeHtml(r.participant.name)}</text><rect x="${left}" y="${y}" width="${w}" height="23" rx="6" fill="${color}" opacity="${r.isTemporary ? 0.55 : 0.9}" ${r.isTemporary ? 'stroke="#f59e0b" stroke-width="2" stroke-dasharray="4 3"' : ""}/><text x="${r.returnPct >= 0 ? xx + 6 : xx - 6}" y="${y + 16}" text-anchor="${r.returnPct >= 0 ? "start" : "end"}" font-size="11" font-weight="800" fill="#344054">${fmtPct(r.returnPct)}</text>`;
  }).join("");
  return `<div class="chart-box"><svg viewBox="0 0 ${width} ${height}"><line x1="${zero}" y1="${m.t}" x2="${zero}" y2="${height - m.b}" stroke="#98a2b3" stroke-dasharray="4 4"/>${bars}<text x="${m.l}" y="${height - 9}" font-size="10" fill="#667085">${fmtDate(round2.pickDate)} \xB7 \u865A\u7EBF\u67F1\u4E3A\u4E34\u65F6\u6210\u7EE9</text></svg></div>`;
}
function renderTotal() {
  const rows = totalRows(), el = document.getElementById("totalTable");
...
function renderSummarySelect() {
  const el = document.getElementById("summaryParticipant"), current = el.value;
  el.innerHTML = state.participants.length ? state.participants.map((p) => `<option value="${p.id}">${escapeHtml(p.name)}</option>`).join("") : '<option value="">\u6682\u65E0\u53C2\u8D5B\u8005</option>';
  if (state.participants.some((p) => p.id === current)) el.value = current;
}
function participantSummary(id) {
  const participant = state.participants.find((p) => p.id === id);
  if (!participant) return null;
  const records = state.rounds.sort((a, b) => String(a.pickDate).localeCompare(String(b.pickDate))).map((round2) => {
    const pick = getPick(round2.id, id), info = returnInfo(pick), row = dailyRows(round2.id).find((r) => r.participant.id === id);
    return { round: round2, pick, info, row, official: round2.status === "settled" && pick && !info.isTemporary && info.value !== null };
  });
  const official = records.filter((r) => r.official), returns = official.map((r) => r.info.value);
  let eq = 1;
  returns.forEach((v) => eq *= 1 + v / 100);
  const sectors = {};
  state.picks.filter((p) => p.participantId === id).forEach((p) => sectors[p.sector || "\u672A\u586B\u5199"] = (sectors[p.sector || "\u672A\u586B\u5199"] || 0) + 1);
  return { participant, records, official, returns, compound: (eq - 1) * 100, winRate: returns.length ? returns.filter((v) => v > 0).length / returns.length * 100 : 0, avg: returns.length ? returns.reduce((a, b) => a + b, 0) / returns.length : null, best: returns.length ? Math.max(...returns) : null, worst: returns.length ? Math.min(...returns) : null, sectors: Object.entries(sectors).sort((a, b) => b[1] - a[1]), avgTech: (() => {
    const a = state.picks.filter((p) => p.participantId === id && p.analysisSummary).map((p) => p.analysisSummary.score);
    return a.length ? a.reduce((x, y) => x + y, 0) / a.length : null;
  })() };
}
function renderSummary() {
  const el = document.getElementById("summaryContent"), id = document.getElementById("summaryParticipant").value, data = participantSummary(id);
  if (!data) {
    el.innerHTML = '<div class="card empty">\u6682\u65E0\u53C2\u8D5B\u8005</div>';
    return;
  }
  const s = data;
  el.innerHTML = `<div class="card" style="margin-bottom:16px"><h3 style="margin-top:0">${escapeHtml(s.participant.name)}</h3><div class="summary-grid"><div class="summary-item"><span>\u7D2F\u8BA1\u6536\u76CA</span><strong class="${pctClass(s.compound)}">${fmtPct(s.compound)}</strong></div><div class="summary-item"><span>\u80DC\u7387</span><strong>${fmtPct(s.winRate)}</strong></div><div class="summary-item"><span>\u65E5\u5747\u6536\u76CA</span><strong>${fmtPct(s.avg)}</strong></div><div class="summary-item"><span>\u5E73\u5747\u6280\u672F\u8BC4\u5206</span><strong>${s.avgTech === null ? "-" : s.avgTech.toFixed(1)}</strong></div><div class="summary-item"><span>\u6700\u4F73\u5355\u65E5</span><strong class="${pctClass(s.best)}">${fmtPct(s.best)}</strong></div><div class="summary-item"><span>\u6700\u5DEE\u5355\u65E5</span><strong class="${pctClass(s.worst)}">${fmtPct(s.worst)}</strong></div><div class="summary-item"><span>\u6B63\u5F0F\u6BD4\u8D5B\u65E5</span><strong>${s.official.length}</strong></div><div class="summary-item"><span>\u4E3B\u8981\u677F\u5757</span><strong>${escapeHtml(s.sectors[0]?.[0] || "-")}</strong></div></div></div><div class="grid grid-2"><div class="card"><div class="section-title"><h3>\u677F\u5757\u504F\u597D</h3></div>${s.sectors.length ? s.sectors.slice(0, 8).map(([name, count]) => `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--line)"><span>${escapeHtml(name)}</span><strong>${count}\u6B21</strong></div>`).join("") : '<div class="empty">\u6682\u65E0\u677F\u5757\u6570\u636E</div>'}</div><div class="card"><div class="section-title"><h3>\u8FD1\u671F\u8BB0\u5F55</h3></div>${s.records.slice(-8).reverse().map((r) => `<div style="display:flex;justify-content:space-between;gap:12px;padding:8px 0;border-bottom:1px solid var(--line)"><span>${fmtDate(r.round.pickDate)} ${r.pick ? escapeHtml(r.pick.stockCode) : "\u672A\u63D0\u4EA4"}</span><strong class="${pctClass(r.info.value)}">${fmtPct(r.info.value)}</strong></div>`).join("") || '<div class="empty">\u6682\u65E0\u8BB0\u5F55</div>'}</div></div>`;
}
function renderData() {
  document.getElementById("settingName").value = state.settings.competitionName || "";
  document.getElementById("settingUnsubmitted").value = state.settings.unsubmittedReturn ?? 0;
  document.getElementById("settingDuplicate").value = String(state.settings.allowDuplicate !== false);
  document.getElementById("settingRule").value = state.settings.ruleNote || "";
}
...
function closeModal(id) {
  ...
  setTimeout(applyPendingRemote, 0);
}
...
function showPage(name) {
  ...
  document.getElementById("mobileTitle").textContent = document.querySelector(`#nav button[data-page="${name}"]`)?.textContent.trim() || "\u6BCF\u65E5\u9009\u80A1\u64C2\u53F0";
  document.getElementById("sidebar").classList.remove("show");
  if (name === "summary") renderSummary();
}
function openParticipantModal(id = "") {
  const item = state.participants.find((p) => p.id === id);
  document.getElementById("participantModalTitle").textContent = item ? "\u7F16\u8F91\u53C2\u8D5B\u8005" : "\u6DFB\u52A0\u53C2\u8D5B\u8005";
  document.getElementById("participantId").value = item?.id || "";
  document.getElementById("participantName").value = item?.name || "";
  document.getElementById("participantColor").value = item?.color || "#2563eb";
  document.getElementById("participantStatus").value = item?.status || "active";
  openModal("participantModal");
}
function saveParticipant() {
  const id = document.getElementById("participantId").value, name = document.getElementById("participantName").value.trim();
  if (!name) return showToast("\u8BF7\u8F93\u5165\u59D3\u540D");
  const current = state.participants.find((p) => p.id === id);
  const item = { ...current || {}, id: id || uid("person"), name, color: document.getElementById("participantColor").value, status: document.getElementById("participantStatus").value, createdAt: current?.createdAt || nowISO(), updatedAt: nowISO() };
  if (current) Object.assign(current, item);
  else state.participants.push(item);
  closeModal("participantModal");
  markChanged("\u53C2\u8D5B\u8005\u5DF2\u4FDD\u5B58");
}
function deleteParticipant(id) {
  const p = state.participants.find((x) => x.id === id);
  if (!p || !confirm(`\u5220\u9664\u53C2\u8D5B\u8005\u201C${p.name}\u201D\u53CA\u5176\u5168\u90E8\u9009\u80A1\u8BB0\u5F55\uFF1F`)) return;
  for (const pick of state.picks.filter((x) => x.participantId === id)) {
    removeEntity("picks", pick.id);
  }
  removeEntity("participants", id);
  markChanged("\u53C2\u8D5B\u8005\u5DF2\u5220\u9664");
}
function openRoundModal(id = "") {
  const item = state.rounds.find((r) => r.id === id);
  document.getElementById("roundModalTitle").textContent = item ? "\u7F16\u8F91\u6BD4\u8D5B\u65E5" : "\u65B0\u5EFA\u6BD4\u8D5B\u65E5";
  document.getElementById("roundId").value = item?.id || "";
  document.getElementById("roundPickDate").value = item?.pickDate || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  document.getElementById("roundBuyDate").value = item?.buyDate || "";
  document.getElementById("roundSellDate").value = item?.sellDate || "";
  document.getElementById("roundStatus").value = item?.status || "open";
  openModal("roundModal");
}
function saveRound() {
  const id = document.getElementById("roundId").value, pickDate = document.getElementById("roundPickDate").value, buyDate = document.getElementById("roundBuyDate").value, sellDate = document.getElementById("roundSellDate").value;
  if (!pickDate || !buyDate || !sellDate) return showToast("\u8BF7\u586B\u5199\u4E09\u4E2A\u65E5\u671F");
  const current = state.rounds.find((r) => r.id === id);
  const item = { ...current || {}, id: id || uid("round"), pickDate, buyDate, sellDate, status: document.getElementById("roundStatus").value, createdAt: current?.createdAt || nowISO(), updatedAt: nowISO() };
  if (current) Object.assign(current, item);
  else state.rounds.push(item);
  sharedRoundId = item.id;
  storageSet(SHARED_ROUND_KEY, sharedRoundId);
...
function deleteRound(id) {
  ...
  if (!r || !confirm(`\u5220\u9664 ${fmtDate(r.pickDate)} \u6BD4\u8D5B\u65E5\u53CA\u5168\u90E8\u9009\u80A1\uFF1F`)) return;
  for (const pick of state.picks.filter((x) => x.roundId === id)) {
    removeEntity("picks", pick.id);
  }
  removeEntity("rounds", id);
  if (sharedRoundId === id) sharedRoundId = "";
  markChanged("\u6BD4\u8D5B\u65E5\u5DF2\u5220\u9664");
}
function openPickModal(id = "") {
  const round2 = getRound(sharedRoundId);
  if (!round2) return showToast("\u8BF7\u5148\u65B0\u5EFA\u6BD4\u8D5B\u65E5");
  const item = state.picks.find((p) => p.id === id);
  document.getElementById("pickModalTitle").textContent = item ? "\u7F16\u8F91\u9009\u80A1" : "\u5F55\u5165\u9009\u80A1";
  document.getElementById("pickId").value = item?.id || "";
  document.getElementById("pickParticipant").innerHTML = activeParticipants().map((p) => `<option value="${p.id}">${escapeHtml(p.name)}</option>`).join("");
  document.getElementById("pickParticipant").value = item?.participantId || activeParticipants()[0]?.id || "";
  document.getElementById("pickCode").value = item?.stockCode || "";
  document.getElementById("pickName").value = item?.stockName || "";
  document.getElementById("pickSector").value = item?.sector || "";
  document.getElementById("pickReason").value = item?.reason || "";
  document.getElementById("pickBuyPrice").value = item?.buyPrice ?? "";
  document.getElementById("pickSellPrice").value = item?.sellPrice ?? "";
  document.getElementById("pickCurrentPrice").value = item?.currentPrice ?? "";
  document.getElementById("pickLookupStatus").innerHTML = "";
  openModal("pickModal");
}
async function lookupPickCode() {
  const code = document.getElementById("pickCode").value.trim();
  if (!/^\d{6}$/.test(code)) return showToast("\u8BF7\u8F93\u51656\u4F4D\u80A1\u7968\u4EE3\u7801");
  const status = document.getElementById("pickLookupStatus");
  status.innerHTML = '<span class="badge"><i class="spin">\u27F3</i> \u67E5\u8BE2\u4E2D</span>';
  try {
    const payload = await apiJSON(`/api/quote?code=${encodeURIComponent(code)}`);
    const q = payload.data;
    document.getElementById("pickName").value = q.name || "";
    document.getElementById("pickSector").value = q.sector || "";
    document.getElementById("pickCurrentPrice").value = q.currentPrice ?? "";
    status.innerHTML = `<span class="badge success">${escapeHtml(q.provider)} \xB7 ${fmtPrice(q.currentPrice)}</span>`;
  } catch (error) {
    status.innerHTML = `${errorButton(error.message)} <span class="muted">\u4ECD\u53EF\u624B\u5DE5\u586B\u5199</span>`;
  }
}
function savePick() {
  const id = document.getElementById("pickId").value, participantId = document.getElementById("pickParticipant").value, stockCode = document.getElementById("pickCode").value.trim();
  if (!participantId || !/^\d{6}$/.test(stockCode)) return showToast("\u8BF7\u9009\u62E9\u53C2\u8D5B\u8005\u5E76\u586B\u51996\u4F4D\u4EE3\u7801");
  const existingForPerson = getPick(sharedRoundId, participantId);
  if (existingForPerson && existingForPerson.id !== id) return showToast("\u8BE5\u9009\u624B\u5F53\u524D\u6BD4\u8D5B\u65E5\u5DF2\u6709\u9009\u80A1");
  if (state.settings.allowDuplicate === false && state.picks.some((p) => p.roundId === sharedRoundId && p.stockCode === stockCode && p.id !== id)) return showToast("\u5F53\u524D\u89C4\u5219\u4E0D\u5141\u8BB8\u91CD\u590D\u9009\u80A1");
  const current = state.picks.find((p) => p.id === id);
  const item = { ...current || {}, id: id || uid("pick"), roundId: sharedRoundId, participantId, stockCode, stockName: document.getElementById("pickName").value.trim(), sector: document.getElementById("pickSector").value.trim(), reason: document.getElementById("pickReason").value.trim(), buyPrice: numeric(document.getElementById("pickBuyPrice").value), sellPrice: numeric(document.getElementById("pickSellPrice").value), currentPrice: numeric(document.getElementById("pickCurrentPrice").value), createdAt: current?.createdAt || nowISO(), updatedAt: nowISO(), marketError: "" };
  if (current) Object.assign(current, item);
...
function deletePick(id) {
  ...
  if (!p || !confirm(`\u5220\u9664 ${p.stockCode} ${p.stockName || ""}\uFF1F`)) return;
  removeEntity("picks", id);
  technicalCache.delete(id);
  markChanged("\u9009\u80A1\u5DF2\u5220\u9664");
}
function updatePickPrice(id, field, value) {
  const p = state.picks.find((x) => x.id === id);
  if (!p) return;
  p[field] = numeric(value);
  p.updatedAt = nowISO();
  markChanged("\u4EF7\u683C\u5DF2\u66F4\u65B0");
}
function settleCurrentRound() {
  const round2 = getRound(sharedRoundId);
  if (!round2) return;
  const missing = state.picks.filter((p) => p.roundId === round2.id && numeric(p.sellPrice) === null);
  if (missing.length) return showToast(`\u4ECD\u6709 ${missing.length} \u53EA\u80A1\u7968\u7F3A\u5C11T+2\u5F00\u76D8\u4EF7`);
  round2.status = "settled";
  round2.updatedAt = nowISO();
  markChanged("\u6BD4\u8D5B\u65E5\u5DF2\u6B63\u5F0F\u7ED3\u7B97");
}
async function refreshPickQuote(id, showMessage = false) {
  const pick = state.picks.find((p) => p.id === id);
  const round2 = getRound(pick?.roundId);
  if (!pick || !round2) return;
  try {
    pick.marketError = "";
    const [quoteResult, historyResult] = await Promise.allSettled([apiJSON(`/api/quote?code=${pick.stockCode}`), apiJSON(`/api/history?code=${pick.stockCode}&start=${round2.buyDate}&end=${round2.sellDate}&limit=20`)]);
    if (quoteResult.status === "fulfilled") {
      const q = quoteResult.value.data;
      pick.stockName = q.name || pick.stockName;
      pick.sector = q.sector || pick.sector;
      pick.currentPrice = numeric(q.currentPrice) ?? pick.currentPrice;
      pick.quoteTime = q.quoteTime || nowISO();
      pick.marketProvider = q.provider;
    }
    if (historyResult.status === "fulfilled") {
      const rows = historyResult.value.data.rows || [];
      const map = new Map(rows.map((r) => [r.date, numeric(r.open)]));
      if (numeric(pick.buyPrice) === null && map.get(round2.buyDate)) pick.buyPrice = map.get(round2.buyDate);
      if (numeric(pick.sellPrice) === null && map.get(round2.sellDate)) pick.sellPrice = map.get(round2.sellDate);
    }
    if (quoteResult.status === "rejected" && historyResult.status === "rejected") throw new Error(`${quoteResult.reason.message}\uFF1B${historyResult.reason.message}`);
    pick.updatedAt = nowISO();
    markChanged("\u884C\u60C5\u5DF2\u66F4\u65B0");
    if (showMessage) showToast("\u884C\u60C5\u5DF2\u66F4\u65B0");
  } catch (error) {
    pick.marketError = error.message;
    pick.updatedAt = nowISO();
    markChanged("\u884C\u60C5\u66F4\u65B0\u5931\u8D25");
    if (showMessage) showToast("\u884C\u60C5\u5931\u8D25\uFF0C\u8BF7\u70B9\u51FB\u9519\u8BEF\u6309\u94AE\u67E5\u770B");
  }
}
async function refreshRoundQuotes() {
  const picks = state.picks.filter((p) => p.roundId === sharedRoundId);
  if (!picks.length) return showToast("\u5F53\u524D\u6BD4\u8D5B\u65E5\u6682\u65E0\u9009\u80A1");
  showToast(`\u6B63\u5728\u5237\u65B0 ${picks.length} \u53EA\u80A1\u7968`);
  for (let i = 0; i < picks.length; i += 3) await Promise.all(picks.slice(i, i + 3).map((p) => refreshPickQuote(p.id, false)));
  showToast("\u884C\u60C5\u5237\u65B0\u5B8C\u6210");
}
function avg(values) {
...
function periodReturn(values, period) {
  return values.length > period && values.at(-1 - period) ? (values.at(-1) / values.at(-1 - period) - 1) * 100 : null;
}
function std(values) {
  const m = avg(values);
  return m === null ? null : Math.sqrt(values.reduce((s, v) => s + (v - m) ** 2, 0) / values.length);
}
function round(value, d = 2) {
  return Number.isFinite(Number(value)) ? Number(Number(value).toFixed(d)) : null;
}
function buildAnalysis(rows, benchmarkRows = []) {
  ...
  const mas = Object.fromEntries(periods.map((p) => [p, sma(closes, p)]));
  const ema12 = emaSeries(closes, 12), ema26 = emaSeries(closes, 26), dif = closes.map((_, i) => ema12[i] - ema26[i]), dea = emaSeries(dif, 9), macd = dif.map((v, i) => (v - dea[i]) * 2);
  ...
  const add = (label, points) => {
    if (Number.isFinite(points) && Math.abs(points) > 0.01) {
      raw += points;
      breakdown.push({ label, points: round(points, 1) });
    }
  };
  const clamp = (v, a, b) => Math.max(a, Math.min(b, v));
  if (deviation20 !== null) add("\u4EF7\u683C\u76F8\u5BF9MA20", clamp(deviation20 * 0.55, -8, 6));
  let aligned = 0, inverted = 0;
  for (let i = 0; i < periods.length - 1; i++) {
    if (!Number.isFinite(mas[periods[i]]) || !Number.isFinite(mas[periods[i + 1]])) continue;
    if (mas[periods[i]] > mas[periods[i + 1]]) aligned++;
    else inverted++;
  }
  add("\u5747\u7EBF\u6392\u5217", clamp((aligned - inverted) * 1.6, -11.2, 11.2));
  for (const [p, up, down] of [[50, 4, -5], [90, 4, -5], [180, 5, -6]]) if (Number.isFinite(mas[p])) add(`\u4EF7\u683C\u76F8\u5BF9MA${p}`, latest.close >= mas[p] ? up : down);
  const spread = latest.close ? (dif.at(-1) - dea.at(-1)) / latest.close * 100 : 0;
  add("MACD", clamp(spread * 20, -8, 8));
  if (rsi14 !== null) {
    let pts = rsi14 >= 55 && rsi14 <= 68 ? 7 : rsi14 > 75 ? -clamp((rsi14 - 75) * 1.2, 2, 12) : rsi14 < 40 ? -clamp((40 - rsi14) * 0.8 + 4, 4, 12) : 2;
    add("RSI14", pts);
  }
  if (r5 !== null) add("\u8FD15\u65E5\u52A8\u91CF", clamp(r5 * 0.7, -7, 7));
  if (r20 !== null) add("\u8FD120\u65E5\u52A8\u91CF", clamp(r20 * 0.35, -9, 9));
  if (r60 !== null) add("\u8FD160\u65E5\u52A8\u91CF", clamp(r60 * 0.12, -6, 6));
  if (relative20 !== null) add("\u76F8\u5BF9\u4E0A\u8BC1", clamp(relative20 * 0.45, -8, 8));
  if (volumeRatio !== null) add("\u91CF\u4EF7", latest.close >= previous.close && volumeRatio >= 1.05 && volumeRatio <= 1.9 ? 5 : latest.close < previous.close && volumeRatio >= 1.5 ? -6 : 0);
  if (volatility !== null && volatility > 4) add("\u6CE2\u52A8\u7387", -clamp((volatility - 4) * 2, 2, 10));
  if (deviation20 > 12) add("\u5747\u7EBF\u504F\u79BB", deviation20 > 18 ? -10 : -5);
  const score = Math.max(0, Math.min(120, Math.round(raw > 100 ? 100 + (raw - 100) * 0.4 : raw)));
  const level = score >= 108 ? "\u6781\u5F3A" : score >= 92 ? "\u5F3A\u52BF" : score >= 76 ? "\u504F\u5F3A" : score >= 58 ? "\u4E2D\u6027" : score >= 42 ? "\u504F\u5F31" : "\u5F31\u52BF";
  const benchMap = new Map(benchmarkRows.map((r) => [r.date, Number(r.close)]));
  const chartRows = rows.slice(-180), baseStock = chartRows[0]?.close, baseBench = benchMap.get(chartRows[0]?.date);
  const chart = chartRows.map((row, index) => {
    const absolute = rows.length - chartRows.length + index, item = { date: row.date, close: round(row.close, 3), dif: round(dif[absolute], 4), dea: round(dea[absolute], 4), macd: round(macd[absolute], 4) };
    for (const p of periods) item[`ma${p}`] = absolute + 1 >= p ? round(avg(closes.slice(absolute + 1 - p, absolute + 1)), 3) : null;
    const b = benchMap.get(row.date);
    item.shIndex = Number.isFinite(b) ? round(b, 2) : null;
    item.shScaled = Number.isFinite(b) && baseBench && baseStock ? round(baseStock * b / baseBench, 3) : null;
    return item;
  });
...
async function analyzePick(id, openAfter = true) {
  const pick = state.picks.find((p) => p.id === id), roundInfo = getRound(pick?.roundId);
  if (!pick || !roundInfo) return;
  try {
    if (openAfter) {
      openModal("technicalModal");
      document.getElementById("technicalBody").innerHTML = '<div class="empty"><i class="spin">\u27F3</i> \u6B63\u5728\u83B7\u53D6\u5386\u53F2\u65E5\u7EBF\u5E76\u8BA1\u7B97\u2026</div>';
    }
    const start = new Date(roundInfo.pickDate);
    ...
    const full = buildAnalysis(stock.data.rows, bench.data.rows);
    full.provider = stock.data.provider;
    full.benchmarkProvider = bench.data.provider;
    technicalCache.set(id, full);
    pick.analysisSummary = { score: full.score, level: full.level, dataDate: full.dataDate, rsi14: full.rsi14, macd: full.macd, return5: full.return5, return20: full.return20, relative20: full.relative20, generatedAt: nowISO() };
    pick.updatedAt = nowISO();
    markChanged("\u6280\u672F\u5206\u6790\u5DF2\u66F4\u65B0");
    if (openAfter) renderTechnicalModal(pick, full);
  } catch (error) {
    pick.marketError = `\u6280\u672F\u5206\u6790\uFF1A${error.message}`;
    pick.updatedAt = nowISO();
    markChanged("\u6280\u672F\u5206\u6790\u5931\u8D25");
    if (openAfter) document.getElementById("technicalBody").innerHTML = `<div class="notice online-warning">\u5206\u6790\u5931\u8D25 ${errorButton(error.message)}</div>`;
  }
}
async function openTechnical(id) {
  const pick = state.picks.find((p) => p.id === id);
  if (!pick) return;
  openModal("technicalModal");
  document.getElementById("technicalTitle").textContent = `${pick.stockCode} ${pick.stockName || ""}\uFF5C\u6280\u672F\u5206\u6790`;
  const cached = technicalCache.get(id);
  if (cached) renderTechnicalModal(pick, cached);
  else await analyzePick(id, true);
}
function toggleTechLayer(id, key) {
  techLayers[key] = !techLayers[key];
  storageSet(TECH_LAYER_KEY, JSON.stringify(techLayers));
  const pick = state.picks.find((p) => p.id === id), full = technicalCache.get(id);
  if (pick && full) renderTechnicalModal(pick, full);
}
function renderTechnicalModal(pick, a) {
  const body = document.getElementById("technicalBody");
  const defs = { ma5: ["MA5", "#f59e0b"], ma10: ["MA10", "#10b981"], ma20: ["MA20", "#8b5cf6"], ma30: ["MA30", "#ec4899"], ma50: ["MA50", "#14b8a6"], ma90: ["MA90", "#6366f1"], ma105: ["MA105", "#a855f7"], ma180: ["MA180", "#64748b"], macd: ["MACD", "#0f766e"], shIndex: ["\u4E0A\u8BC1\u6307\u6570", "#dc2626"] };
  const controls = Object.entries(defs).map(([key, [label, color]]) => `<button class="curve-btn ${techLayers[key] ? "active" : ""}" style="--c:${color}" onclick="toggleTechLayer('${pick.id}','${key}')"><i class="swatch"></i>${label}</button>`).join("");
  body.innerHTML = `<div class="grid grid-4"><div class="card stat"><div class="label">\u6280\u672F\u8BC4\u5206</div><div class="value">${a.score}/120</div><div class="sub">${a.level}</div></div><div class="card stat"><div class="label">RSI14</div><div class="value">${a.rsi14 ?? "-"}</div><div class="sub">MACD ${a.macd ?? "-"}</div></div><div class="card stat"><div class="label">\u8FD120\u65E5</div><div class="value ${pctClass(a.return20)}">${fmtPct(a.return20)}</div><div class="sub">\u76F8\u5BF9\u4E0A\u8BC1 ${fmtPct(a.relative20)}</div></div><div class="card stat"><div class="label">\u91CF\u6BD4\u4F30\u7B97</div><div class="value">${a.volumeRatio ?? "-"}</div><div class="sub">\u6570\u636E\u81F3 ${fmtDate(a.dataDate)}</div></div></div><div style="margin-top:16px">${controls}</div><div id="technicalChartHost" style="margin-top:10px"></div><div class="card" style="margin-top:14px"><div class="section-title"><h3>\u4E3B\u8981\u8BC4\u5206\u9879</h3><button class="btn btn-small" onclick="analyzePick('${pick.id}',true)">\u91CD\u65B0\u5206\u6790</button></div><div class="actions">${a.breakdown.slice(0, 14).map((x) => `<span class="badge ${x.points >= 0 ? "success" : "danger"}">${escapeHtml(x.label)} ${x.points >= 0 ? "+" : ""}${x.points.toFixed(1)}</span>`).join("")}</div><p class="muted" style="font-size:11px">\u4E2A\u80A1\uFF1A${escapeHtml(a.provider || "-")}\uFF1B\u6307\u6570\uFF1A${escapeHtml(a.benchmarkProvider || "-")}</p></div>`;
  renderTechChart(pick, a, defs);
}
function renderTechChart(pick, a, defs) {
  const points = a.chart || [];
  const host = document.getElementById("technicalChartHost");
  if (points.length < 2) {
    host.innerHTML = '<div class="empty">\u6682\u65E0\u56FE\u8868\u6570\u636E</div>';
    return;
  }
  ...
  const series = [{ key: "close", label: "\u6536\u76D8\u4EF7", color: "#2563eb", w: 2.8 }];
  for (const key of Object.keys(defs)) {
    if (!techLayers[key] || key === "macd") continue;
    if (key === "shIndex") series.push({ key: "shScaled", label: "\u4E0A\u8BC1\u6307\u6570", color: defs[key][1], w: 2, dash: "7 5" });
    else series.push({ key, label: defs[key][0], color: defs[key][1], w: 1.8 });
  }
  const vals = series.flatMap((s) => points.map((p) => Number(p[s.key]))).filter(Number.isFinite);
  let min = Math.min(...vals), max = Math.max(...vals);
  if (min === max) {
    min -= 1;
    max += 1;
  ...
  min -= margin;
  max += margin;
  const span = max - min, x = (i) => pad + i / (points.length - 1) * plotW, y = (v) => bottom - (v - min) / span * (bottom - top);
  const grids = Array.from({ length: 5 }, (_, i) => {
    const yy = top + i / 4 * (bottom - top), v = max - i / 4 * span;
    return `<line x1="${pad}" y1="${yy}" x2="${width - 18}" y2="${yy}" stroke="#e4e8f0"/><text x="${pad - 6}" y="${yy + 3}" text-anchor="end" font-size="10" fill="#98a2b3">${fmtPrice(v)}</text>`;
  }).join("");
  const lines = series.map((s) => {
    let d = "";
    points.forEach((p, i) => {
      const v = Number(p[s.key]);
      if (Number.isFinite(v)) d += `${d ? "L" : "M"}${x(i)},${y(v)} `;
    });
    return `<path d="${d}" fill="none" stroke="${s.color}" stroke-width="${s.w}" ${s.dash ? `stroke-dasharray="${s.dash}"` : ""} stroke-linecap="round" stroke-linejoin="round"/>`;
  }).join("");
  let macd = "", macdCfg = null;
  if (techLayers.macd) {
    const mt = mainH + gap, mb = height - 22, mvals = points.flatMap((p) => [p.macd, p.dif, p.dea]).filter(Number.isFinite), abs = Math.max(1e-3, ...mvals.map(Math.abs)), my = (v) => mt + (abs - v) / (abs * 2) * (mb - mt), zero = my(0), barW = Math.max(1, Math.min(4, plotW / points.length * 0.65));
    const bars = points.map((p, i) => Number.isFinite(Number(p.macd)) ? `<rect x="${x(i) - barW / 2}" y="${Math.min(zero, my(p.macd))}" width="${barW}" height="${Math.max(1, Math.abs(my(p.macd) - zero))}" fill="${p.macd >= 0 ? "#ef4444" : "#10b981"}" opacity=".65"/>` : "").join("");
    const line = (key) => {
      const d = points.map((p, i) => Number.isFinite(Number(p[key])) ? `${i ? "L" : "M"}${x(i)},${my(p[key])}` : "").join(" ");
      return `<path d="${d}" fill="none" stroke="${key === "dif" ? "#2563eb" : "#f59e0b"}" stroke-width="1.7"/>`;
    };
    macd = `<text x="${pad}" y="${mt - 6}" font-size="11" font-weight="800" fill="#475467">MACD</text><line x1="${pad}" y1="${zero}" x2="${width - 18}" y2="${zero}" stroke="#98a2b3" stroke-dasharray="4 4"/>${bars}${line("dif")}${line("dea")}`;
    macdCfg = { mt, mb, abs, my };
  }
  ...
  hit.addEventListener("pointermove", (event) => {
    const rect = svg.getBoundingClientRect(), vx = (event.clientX - rect.left) / rect.width * width, index = Math.max(0, Math.min(points.length - 1, Math.round((vx - pad) / plotW * (points.length - 1)))), p = points[index], cx = x(index);
    ...
    for (const s of series) {
      const v = Number(p[s.key]);
      if (Number.isFinite(v)) dots.push(`<circle cx="${cx}" cy="${y(v)}" r="4" fill="${s.color}" stroke="#fff" stroke-width="1.5"/>`);
    }
    if (macdCfg) {
      for (const [key, color] of [["dif", "#2563eb"], ["dea", "#f59e0b"]]) if (Number.isFinite(Number(p[key]))) dots.push(`<circle cx="${cx}" cy="${macdCfg.my(p[key])}" r="3.5" fill="${color}" stroke="#fff" stroke-width="1.4"/>`);
    }
    markers.innerHTML = dots.join("");
    const rows = [];
    for (const s of series) {
      if (s.key === "shScaled") {
        if (Number.isFinite(Number(p.shIndex))) rows.push([s.color, "\u4E0A\u8BC1\u6307\u6570", Number(p.shIndex).toFixed(2)]);
      } else if (Number.isFinite(Number(p[s.key]))) rows.push([s.color, s.label, Number(p[s.key]).toFixed(3)]);
    }
    if (macdCfg) {
      for (const [key, label, color] of [["dif", "DIF", "#2563eb"], ["dea", "DEA", "#f59e0b"], ["macd", "MACD", Number(p.macd) >= 0 ? "#ef4444" : "#10b981"]]) if (Number.isFinite(Number(p[key]))) rows.push([color, label, Number(p[key]).toFixed(4)]);
    }
    tooltip.innerHTML = `<div class="tooltip-date">${p.date}</div>${rows.map(([c, l, v]) => `<div class="tooltip-row"><i class="tooltip-dot" style="background:${c}"></i><span>${l}</span><strong>${v}</strong></div>`).join("")}`;
    tooltip.classList.add("show");
    const br = box.getBoundingClientRect();
    let left = event.clientX - br.left + 12;
    if (left + tooltip.offsetWidth > box.clientWidth - 8) left = event.clientX - br.left - tooltip.offsetWidth - 12;
    let tt = event.clientY - br.top - tooltip.offsetHeight / 2;
    tt = Math.max(8, Math.min(tt, box.clientHeight - tooltip.offsetHeight - 8));
...
function saveSettings() {
  state.settings.competitionName = document.getElementById("settingName").value.trim() || "\u6BCF\u65E5\u9009\u80A1\u64C2\u53F0";
  state.settings.unsubmittedReturn = Number(document.getElementById("settingUnsubmitted").value || 0);
  state.settings.allowDuplicate = document.getElementById("settingDuplicate").value === "true";
  state.settings.ruleNote = document.getElementById("settingRule").value.trim();
...
function exportDailyCSV() {
  const round2 = getRound(sharedRoundId);
  if (!round2) return;
  const rows = [["\u540D\u6B21", "\u9009\u624B", "\u80A1\u7968\u4EE3\u7801", "\u80A1\u7968\u540D\u79F0", "\u6536\u76CA\u7387%", "\u79EF\u5206", "\u72B6\u6001"], ...dailyRows(round2.id).map((r) => [r.rank, r.participant.name, r.pick?.stockCode || "", r.pick?.stockName || "", r.returnPct ?? "", r.points, r.isTemporary ? "\u4E34\u65F6" : r.isMissing ? "\u5F85\u4EF7\u683C" : r.isUnsubmitted ? "\u672A\u63D0\u4EA4" : "\u6B63\u5F0F"])];
  csvDownload(`\u6BCF\u65E5\u6392\u540D_${round2.pickDate}.csv`, rows);
}
function exportTotalCSV() {
  const rows = [["\u603B\u6392\u540D", "\u9009\u624B", "\u7D2F\u8BA1\u6536\u76CA\u7387%", "\u603B\u79EF\u5206", "\u6B63\u5F0F\u6BD4\u8D5B\u65E5", "\u51A0\u519B\u6B21\u6570"], ...totalRows().map((r) => [r.rank, r.participant.name, r.compoundReturn, r.points, r.days, r.wins])];
  csvDownload("\u603B\u6392\u884C\u699C.csv", rows);
}
function exportJSON() {
  downloadBlob(`\u80A1\u7968\u6BD4\u8D5B\u5171\u4EAB\u5907\u4EFD_${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`, new Blob([JSON.stringify(state, null, 2)], { type: "application/json" }));
}
async function importJSON(file) {
  if (!file) return;
  try {
    const parsed = normalizeState(JSON.parse(await file.text()));
    if (!confirm("\u5BFC\u5165\u4F1A\u8986\u76D6\u5E76\u5408\u5E76\u5F53\u524D\u5171\u4EAB\u6570\u636E\uFF0C\u662F\u5426\u7EE7\u7EED\uFF1F")) return;
    state = mergeStates(parsed, state);
    markChanged("\u5907\u4EFD\u5DF2\u5BFC\u5165");
  } catch (error) {
...
function resetSharedData() {
  if (!confirm("\u786E\u5B9A\u6E05\u7A7A\u5168\u90E8\u5728\u7EBF\u5171\u4EAB\u6570\u636E\uFF1F\u6B64\u64CD\u4F5C\u4F1A\u5F71\u54CD\u6240\u6709\u8BBF\u95EE\u8005\u3002")) return;
  if (!confirm("\u518D\u6B21\u786E\u8BA4\uFF1A\u53C2\u8D5B\u8005\u3001\u6BD4\u8D5B\u65E5\u548C\u9009\u80A1\u8BB0\u5F55\u90FD\u5C06\u6E05\u7A7A\u3002")) return;
  state = defaultState();
  state.meta.settingsUpdatedAt = nowISO();
  dirty = true;
  saveLocal();
...
function errorButton(message) {
  return message ? `<button class="error-btn" data-error="${escapeHtml(message)}" onclick="showError(event,this)">\u9519\u8BEF\uFF01</button>` : "";
}
function showError(event, button) {
  event?.stopPropagation();
  const pop = document.getElementById("errorPopover"), body = document.getElementById("errorPopoverBody");
  body.textContent = button.dataset.error || "\u672A\u77E5\u9519\u8BEF";
  pop.classList.add("show");
  requestAnimationFrame(() => {
    const r = button.getBoundingClientRect(), pr = pop.getBoundingClientRect();
    let left = Math.max(12, Math.min(r.right - pr.width, innerWidth - pr.width - 12)), top = r.bottom + 7;
    if (top + pr.height > innerHeight - 12) top = r.top - pr.height - 7;
    pop.style.left = `${left}px`;
    pop.style.top = `${Math.max(12, top)}px`;
  });
}
function closeErrorPopover() {
  document.getElementById("errorPopover").classList.remove("show");
}
Object.assign(window, { showPage, setSharedRound, openParticipantModal, saveParticipant, deleteParticipant, openRoundModal, saveRound, deleteRound, openPickModal, lookupPickCode, savePick, deletePick, updatePickPrice, settleCurrentRound, refreshPickQuote, refreshRoundQuotes, openTechnical, analyzePick, toggleTechLayer, closeModal, saveSettings, exportDailyCSV, exportTotalCSV, exportJSON, importJSON, resetSharedData, showError, closeErrorPopover });
...
document.getElementById("visitorName").addEventListener("change", (event) => {
  visitorName = event.target.value.trim() || `\u8BBF\u5BA2-${clientId().slice(-4).toUpperCase()}`;
  event.target.value = visitorName;
  storageSet(CLIENT_NAME_KEY, visitorName);
});
document.addEventListener("click", (event) => {
  if (document.getElementById("errorPopover").classList.contains("show") && !event.target.closest("#errorPopover") && !event.target.closest(".error-btn")) closeErrorPopover();
});
...
document.addEventListener("focusout", () => setTimeout(applyPendingRemote, 80));
renderAll();
loadRemote({ initial: true }).finally(() => startRealtime());
pollTimer = setInterval(pollRemote, 15e3);
